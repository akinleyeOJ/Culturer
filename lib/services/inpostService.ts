...
export const fetchInPostPointPayloads = async ({
    ...
    try {
        ...
        if (!data?.success) throw new Error(data?.error || 'Failed to fetch InPost pickup points');
        ...
        if (__DEV__) {
            console.warn('InPost pickup-point lookup unavailable, falling back.', err?.message || err);
        }
...
export const fetchInPostCalculatedRates = async (shipments: InPostRateShipmentRequest[]) => {
    try {
        ...
        if (error) throw error;
        if (!data?.success) throw new Error(data?.error || 'Failed to calculate InPost rates');

        return {
            success: true,
            rates: Array.isArray(data?.rates) ? (data.rates as InPostCalculatedRate[]) : [],
        };
    } catch (err: any) {
        if (__DEV__) {

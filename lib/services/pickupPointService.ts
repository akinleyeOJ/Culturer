...
const looksLikePostalCode = (value: string) => /^\d{2}-?\d{3}$/.test(value.trim());
...
const inferCitySearchFromQuery = (value: string) => {
    ...
    return beforeNumber.replace(/[^\p{L}\s-]+/gu, ' ').replace(/\s+/g, ' ').trim();
};

const scorePickupPoint = (pickupPoint: PickupPointResult, query: string) => {
    const normalizedQuery = normalizeSearchValue(query);
    const normalizedAddress = normalizeSearchValue(`${pickupPoint.id} ${pickupPoint.address} ${pickupPoint.hint}`);
    ...
    return normalizedQuery
        .split(' ')
        .filter(Boolean)
        .reduce((score, token) => score + (normalizedAddress.includes(token) ? 100 : 0), 0);
};
...
const sortPickupPoints = (
    ...
    const hasCoordinates = typeof context.latitude === 'number' && typeof context.longitude === 'number';
    ...
    const queryCandidates = Array.from(
        new Set(
            [
                ...
                looksLikePostalCode(postalCode) ? formatPolishPostalCode(postalCode) : '',
                city,
                inferredCity,
                addressQuery,
            ].filter(Boolean)
    ...
    return Array.from(
        new Set(
            queryCandidates.flatMap((candidate) => {
                const urls = [`${INPOST_POINTS_API}?query=${encodeURIComponent(candidate)}&per_page=25`];
...
export const fetchPickupPoints = async (
    ...
    const city = context.city?.trim() || fallbackAddress?.city?.trim() || '';
    const postalCode = context.postalCode?.trim() || fallbackAddress?.postalCode?.trim() || '';
    const country = context.country?.trim() || fallbackAddress?.country?.trim() || '';

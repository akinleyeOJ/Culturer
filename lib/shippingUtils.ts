...
export const COUNTRY_PROVIDER_TEMPLATES: Record<string, CarrierTemplate[]> = {
  ...
  Italy: [
    {
      name: "Poste Italiane Punto Poste",
      type: "locker",
      mode: "locker_pickup",
    },
    {
...
export const ZONE_OPTIONS: {
  ...
  { key: "domestic", label: "Domestic", desc: "Ship within your country" },
  {
    ...
    desc: "Ship to EU countries (no customs)",
  },
  {
    key: "worldwide",
    label: "Worldwide",
    desc: "Ship globally (customs may apply)",
  },
];

export const DEFAULT_SHIPPING_CONFIG: SellerShippingConfig = {
  processing_time: "3-5 business days",
  origin_country: SHIPPING_LAUNCH_COUNTRY,
  origin_street1: "",
  origin_city: "",
  origin_state: "",
  origin_zip: "",
...
const normalizeProviderMode = (
  carrier: Partial<CarrierConfig>,
): ShippingProviderMode =>
  carrier.mode ||
  (carrier.type === "locker" ? "locker_pickup" : "home_delivery");
...
const filterTemplatesBySupportedLockerProviders = (
  ...
  return templates.filter(
    (template) =>
      template.mode !== "locker_pickup" ||
      supportedProviderNames.has(template.name),
  );
};
...
export const getSupportedLockerProviderNamesForCountry = (
  ...
  return baseTemplates
    ...
    .filter((template) =>
      providerSupportsSendcloudCarrierCodes(template.name, enabledCarrierCodes),
    )
...
export const getProviderTemplatesForCountry = (
  ...
  return isSupportedCountry
    ? filterTemplatesBySupportedLockerProviders(
        FALLBACK_EU_PROVIDER_TEMPLATES,
        supportedLockerProviders,
      )
    : [];
...
export const buildProviderConfigs = (
  ...
  const templates = getProviderTemplatesForCountry(
    country,
    supportedLockerProviders,
  );
  const existingByName = new Map(
    existingProviders.map((provider) => [
      provider.name,
      normalizeCarrier(provider),
    ]),
  );
  const hasExplicitSelections = existingProviders.some(
    (provider) => normalizeCarrier(provider).enabled,
  );
...
const derivePreferredCarriers = (
  ...
  const preferred = Array.isArray(savedNames)
    ? savedNames.filter((name) => validNames.has(name))
    : [];
...
export const hydrateShippingConfig = (
  ...
  const normalizedCountry = normalizeCountryName(
    saved?.origin_country || DEFAULT_SHIPPING_CONFIG.origin_country,
  );
  ...
  const hasExplicitProviderSelections = legacyProviders.some(
    (provider) => normalizeCarrier(provider as CarrierConfig).enabled,
  );
  const inferredHomeEnabled = providers.some(
    (provider) => provider.mode === "home_delivery" && provider.enabled,
  );
  const inferredLockerEnabled = providers.some(
    (provider) => provider.mode === "locker_pickup" && provider.enabled,
  );

  // If locked-default providers force a mode's carriers to enabled, the mode itself
  // must also be enabled — a stale `false` saved before those defaults existed must
  // not silently suppress the whole mode.
  const homeEnabled = inferredHomeEnabled
    ? true
    : (savedModes?.home_delivery?.enabled ?? false);
  const lockerEnabled = inferredLockerEnabled
    ? true
    : (savedModes?.locker_pickup?.enabled ?? false);
  const localPickupEnabled =
    savedModes?.local_pickup?.enabled ?? saved?.local_pickup ?? false;

  const modes: SellerShippingConfig["modes"] = {
    home_delivery: {
      enabled: homeEnabled,
      preferred_carriers: derivePreferredCarriers(
        savedModes?.home_delivery?.preferred_carriers,
        providers,
        "home_delivery",
      ),
    ...
    locker_pickup: {
      ...
      preferred_carriers: derivePreferredCarriers(
        savedModes?.locker_pickup?.preferred_carriers,
        providers,
        "locker_pickup",
      ),
    },
  ...
  return {
    ...
    shipping_zones:
      Array.isArray(saved?.shipping_zones) && saved.shipping_zones.length > 0
...
export const getTrackingUrl = (
  ...
  if (c.includes("dpd"))
    return `https://www.dpd.com/tracking?parcelno=${trackingNumber}`;
  ...
  if (c.includes("evri") || c.includes("hermes"))
    return `https://www.evri.com/track/parcel/${trackingNumber}`;
  if (c.includes("ups"))
    return `https://www.ups.com/track?tracknum=${trackingNumber}`;
  if (c.includes("fedex"))
    return `https://www.fedex.com/fedextrack/?trknbr=${trackingNumber}`;
  if (c.includes("poczta"))
    return `https://śledzenie.poczta-polska.pl/?numer=${trackingNumber}`;
  if (c.includes("gls")) return `https://gls-group.com/track/${trackingNumber}`;
  if (c.includes("postnl"))
    return `https://postnl.nl/tracktrace/?B=${trackingNumber}`;
  if (c.includes("colissimo"))
    return `https://www.laposte.fr/outils/suivre-vos-envois?code=${trackingNumber}`;
  if (c.includes("correos"))
    return `https://www.correos.es/ss/Satellite/site/aplicacion-localizador_702-sidioma=en_GB?numero=${trackingNumber}`;
  if (c.includes("poste italiane"))
    return `https://www.poste.it/cerca/index.html#/risultati-di-ricerca?q=${trackingNumber}`;
  if (c.includes("brt") || c.includes("bartolini"))
    return `https://www.brt.it/tracking?w=auto&ids=${trackingNumber}`;
  return null;
};
...
export const getCarrierPrice = (
  ...
  switch (weightTier) {
    ...
    case "small":
      return carrier.price_small;
    case "medium":
      return carrier.price_medium;
    case "large":
      return carrier.price_large;
    default:
      return carrier.price_medium;
  }
};
...
export const getEnabledCarriers = (
  ...
  return providers.filter(
    (provider) => provider.enabled && isModeEnabled(config, provider.mode),
  );
};
...
export const buildLocalPickupOption = (
  config: SellerShippingConfig,
): CarrierConfig | null => {
